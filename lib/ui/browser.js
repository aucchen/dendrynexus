...
(function($) {
  ...
  BrowserUserInterface.prototype.setBg = function(image_url) {
      if (!image_url || image_url == 'none' || image_url == 'null' || this.disable_bg) {
          ...
          // TODO: add condition for gradient - if image_url.startswith('linear-gradient')
  ...
  // 'queue' for adding the music to a queue.
  // 'replace' for replacing the currently playing music, clearing the queue (default behavior).
  // 'loop' if this music will loop indefinitely.
  // 'nofade' if the sound will be played instantly without a fadein or fadeout.
  // 'fade' if the music will be faded in (default behavior)
  BrowserUserInterface.prototype.audio = function(audio) {
      var audioData = audio.split(' ');
      if (audioData.includes('queue')) {
      }
      if (audioData.includes('loop')) {
      }
      if (audioData.includes('nofade')) {
      }
      audio = audioData[0];
