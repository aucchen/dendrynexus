...
(function($) {
  ...
  BrowserUserInterface.prototype.deleteSlot = function() {
    if (localStorage[this.save_prefix + '_' + slot]) {
  ...
  BrowserUserInterface.prototype.populateSaveSlots = function() {
    ...
    for (var i = 0; i < max_slots; i++) {
      ...
      if (localStorage[this.save_prefix + '_' + i]) {
        var timestamp = localStorage[this.save_prefix + '_timestamp_' + i];
