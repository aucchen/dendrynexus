...
(function() {
    ...
    async.eachSeries(chunks, function(chunk, chunkDone) {
      ...
      validateRelativeId(parts[0], null, function(err, relativeId) {
        var handleError = function(err) {
          return chunkDone(properr(object, null, err.message));
        };
  ...
  // GoToRef properties take zero or more variable names
  // (which should be string values containing absolute scene IDs) followed by
