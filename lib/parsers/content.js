...
(function() {
  ...
  /*
    ...
    **some words** - strong emphasis
    ...
    [? if condition: text ?] - conditional section (TODO: add an if-then-else with an additional colon)
    {! stuff !} - will be displayed as raw html
    ` - code
    \ - escaping
    TODO: links and scene inclusion.
    [[Scene|sceneID]] - link
    [[+ sceneID +]] - include scene text (run onArrival and onDeparture for the included scene? or just include the scene text? easier to just include the text)
    [? if condition: text : text2 ?] - display text if condition is true, text2 if condition is false.
   */

  var featuresRe = /\[[?+]?|[?+]?\]|\{\!|\!\}|\*+|\/\/\n|^[ \t]*\n|^\s*---\s*\n|^\s*>>?|^\s*=|`|\[\[|\]\]|\[\[\+|\+\]\]/mg;
  var escapedFeaturesRe = /(\\)(?=\*+|\/\/\n|^[ \t]*\n|^\s*---\s*\n|^\s*>>?|^\s*=|\[[?+]?|[?+]?\]|\{\!|\!\}|`|\[\[|\]\])/mg;
  ...
  BoundaryFinder.prototype.init = function() {
    ...
    this.inEmphasis = [false, false, false];
  ...
  BoundaryFinder.prototype.process = function(text, features) {
    ...
    for (var i = 0; i < features.length; i++) {
      ...
      case '[+': // Start of insert.
        if (this.inInsert) {
          throw new Error(
            'Can\'t begin a new insert in the middle of an insert. ' + text
          );
        } else {
      ...
      case '[': // Start of hidden text.
        if (this.inHidden) {
          throw new Error(
            'Can\'t begin a new hidden block in the middle of a hidden block. ' + text
          );
        } else if (this.inInsert) {
          throw new Error(
            'Can\'t nest a hidden block in an insert. ' + text
          );
        } else {
      ...
      case '`':
        this.em(3);
        break;
      case '[[': // start of link. TODO
        break;
      case ']]': // End of link. TODO
        break;
      case '|': // separation between link text and link destination.
        break;
      default:
  ...
  var normaliseContent = function(content) {
    if (typeof content == 'string') {
      // remove backslashes...
      content = content.replace(escapedFeaturesRe, ''); 
    }
