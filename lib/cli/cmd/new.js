...
(function() {
  ...
  var getDefaultAuthor = function(data, callback) {
    if (data.author === null) {
      ...
      exec('id -P | cut -f8 -d:', function(err, stdout) {
        var defaultAuthor;
  ...
  // https://stackoverflow.com/posts/2117523/revisions?page=2
  var getDefaultIFID = function(data, callback) {
      try {
          data.ifid = crypto.randomUUID().toUpperCase();
      } catch (err) {
          data.ifid = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
                var r = Math.random()*16|0, v = c == 'x' ? r : (r&0x3|0x8);
                return v.toString(16).toUpperCase();
          });
      }
      console.log('IFID: ', data.ifid);
      callback(null, data);
  };
