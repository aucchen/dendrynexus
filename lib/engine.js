...
(function() {
  ...
  DendryEngine.prototype.displayChoices = function() {
    ...
    if (scene.isHand) {
      // TODO: separate choices into decks and pinned cards
      var decks = [];
      var pinnedCards = [];
      for (var c of choices) {
        ...
        if (choiceScene.isDeck) {
          ...
          c.isDeck = false;
          c.image = choiceScene.cardImage;
          pinnedCards.append(c);
        }
      }
      this.ui.displayDecks(decks);
      this.ui.displayHand(this.currentHand[this.state.sceneId]);
  ...
  DendryEngine.prototype.drawCard = function(deckId) {
    ...
    if (scene.maxCards <= currenHand.length) {
    ...
    card.image = image;
    this.state.currentHands[currentSceneId].append(card);

    // display the hand
    this.ui.displayHand(this.state.currentHands[currentsceneId]);
    return card;
  };


  // TODO: dendrynexus - play a card (remove it from the current hand)
  ...
  // TODO: dendrynexus returns a single available card from the given deck, formatted as an object of the type {id: id, title: title}
